...
- **`@behindthemusictree/app-kit`**: Bumped `1.3.0` → `3.0.0`. `TrackList.uploadedTracks` and related APIs were renamed to their generic `Track` equivalents in app-kit 3.0.0 (this repo never used app-kit's upload-specific exports, so the only fallout is the `.uploadedTracks` → `.tracks` reads in `AutoAdvance.tsx` and `Player.tsx`).
...
- **Uploaded-track timeout**: Removed the dead `NEXT_PUBLIC_TRACK_UPLOAD_TIMEOUT_MS` env var and its `uploadTimeoutMs` plumbing into `<GenreTreeView>`. The `reference` scope has no `.uploaded` library endpoint (grow-api is YouTube-only), so this was already unused for its stated purpose. Dropped from `next.config.js`'s `REQUIRED_ENV_VARS`, the `Dockerfile`'s build `ARG`/`ENV`, and the README/`docs/DEPLOYMENT.md` build-arg examples.
