...
- **`@behindthemusictree/app-kit`**: Bumped `3.0.0` → `4.0.0`, matching app-kit's `TrackListSidebar`/`TrackItem`/`TrackListProvider` becoming generic over `T extends TrackBase`. `TrackListSidebar` no longer takes `getBackendBaseUrl`. `TrackListProvider` now requires `schema`, `listEndpoint`, and `listQueryKey` props, passed as `YoutubeTrackDetailedSchema` and `libraryEndpoints`/`libraryQueryKeys`'s `reference.youtube.list(page)` in `providers.tsx`.
