...
export default function Player({ className }: PlayerProps) {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 z-50 flex max-h-[calc(100vh-2rem)] w-144 flex-col overflow-hidden rounded-lg bg-black shadow-2xl ${!playerTrackObject ? "hidden" : ""} ${className ?? ""}`}
    >
