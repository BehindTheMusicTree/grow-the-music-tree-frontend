...
- **PRs to `develop`** can come from `feature/*`, `chore/*`, `dependabot/*`, `release/*`, `hotfix/*`, or `main` (required for post-release/hotfix back-merges). Other branch types (e.g., `fix/*`, `refactor/*`, etc.) are blocked by the branch protection workflow.
