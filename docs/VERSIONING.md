...
Version bump and changelog roll-up for a shipping release happen **once**, either on `release/*` (preferred) or on `main`, never both — after **`release/*` or `hotfix/*` is merged into `main` via PR** ([CONTRIBUTING.md](../CONTRIBUTING.md) §7). Prepare `package.json` / changelog on `release/*` with **`npm pkg set version=X.Y.Z`** (not `npm version` — its `postversion` script assumes it's amending a commit `npm version` just created, and will instead amend whatever commit is currently `HEAD` if run on `release/*` without a full `npm version` invocation, corrupting shared history). If `release/*` already prepared the bump, just tag `main`'s merge commit directly (no extra bump). Only fall back to running the full `npm version` (bump + commit + tag + `postversion`) directly on `main` if `release/*` did **not** prepare the bump.
